...
class GRU_LSTM_Autoencoder:
    def __init__(self, input_shape=(51, 2), latent_dim=16, lstm_units=[64, 32], gru_units=[32, 16], dropout_rate=0.3, l2_reg=1e-4,use_attention=False):
    ...
    def build_model(self):
        ...
        # --- Encoder ---
        for i in range(len(self.lstm_units)):  #len(self.lstm_units)) == len(self.gru_units)
            return_seq = True if i < len(self.lstm_units) - 1 else False
            # Regularization
            reg = regularizers.l2(self.l2_reg) if self.l2_reg > 0 else None

            x = GRU(self.gru_units[i], return_sequences=return_seq,
                     kernel_regularizer=reg)(x)
            x = BatchNormalization()(x)
            x = Dropout(self.dropout_rate)(x)
            x = LSTM(self.lstm_units[i], return_sequences=return_seq,
                     kernel_regularizer=reg)(x)
            x = BatchNormalization()(x)
            x = Dropout(self.dropout_rate)(x)
        ...
        for i in reversed(range(len(self.lstm_units))):
            x = GRU(self.gru_units[i], return_sequences=True,
                     kernel_regularizer=reg)(x)
            x = BatchNormalization()(x)
            x = Dropout(self.dropout_rate)(x)
            x = LSTM(self.lstm_units[i], return_sequences=True,
                     kernel_regularizer=reg)(x)
            x = BatchNormalization()(x)
            x = Dropout(self.dropout_rate)(x)
