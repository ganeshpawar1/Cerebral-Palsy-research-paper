...
class GRUAutoencoder:
    def __init__(self, input_shape=(51, 2), latent_dim=16, gru_units=[64, 32], 
                 dropout_rate=0.3, l2_reg=1e-4, use_attention=False):
    ...
    def build_model(self):
        ...
        for i, units in enumerate(self.gru_units):
            return_seq = True if i < len(self.gru_units) - 1 else False

            reg = regularizers.l2(self.l2_reg) if self.l2_reg > 0 else None

            x = GRU(units, return_sequences=return_seq,
    ...
    def save(self, model_path='gru_autoencoder_model.h5'):
